...
class NotificationManager {
    ...
    private func scheduleReminder(for task: TaskItem) {
        ...
        let reminderTime = Calendar.current.date(byAdding: .minute, value: -10, to: task.startTime) ?? task.startTime
    ...
    private func scheduleExactTime(for task: TaskItem) {
        ...
        let triggerDate = Calendar.current.dateComponents([.hour, .minute], from: task.startTime)
        let trigger = UNCalendarNotificationTrigger(dateMatching: triggerDate, repeats: false)
        
        let request = UNNotificationRequest(identifier: "\(task.id)-exact", content: content, trigger: trigger)
        UNUserNotificationCenter.current().add(request)
    }
    ...
    private func scheduleOverdue(for task: TaskItem) {
        ...
        let overdueTime = Calendar.current.date(byAdding: .minute, value: 15, to: task.startTime) ?? task.startTime
        let triggerDate = Calendar.current.dateComponents([.hour, .minute], from: overdueTime)
        let trigger = UNCalendarNotificationTrigger(dateMatching: triggerDate, repeats: false)
        
        let request = UNNotificationRequest(identifier: "\(task.id)-overdue", content: content, trigger: trigger)
        UNUserNotificationCenter.current().add(request)
    }
    
    func cancelNotifications(for task: TaskItem) {
        let identifiers = ["\(task.id)-reminder", "\(task.id)-exact", "\(task.id)-overdue"]
