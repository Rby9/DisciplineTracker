...
struct WeeklyInsightsView: View {
    ...
    private var end: Date {
        calendar.date(byAdding: .day, value: 7, to: firstDay) ?? firstDay
    }

    private var weekTasks: [TaskItem] {
        tasks.filter {
            $0.startTime >= firstDay && $0.startTime < end
        }
    }
    ...
    private var pending: Int {
        weekTasks.filter { $0.isPending }.count
    }

    private var skipped: Int {
        weekTasks.filter { $0.isSkipped }.count
    }
    ...
    private var dayBreakdown: some View {
        Section {
            ForEach(days, id: \.self) { day in
                let items = weekTasks.filter {
                    calendar.isDate($0.startTime, inSameDayAs: day)
                }

                Button {
                    journalDay = InsightJournalDay(date: day)
                } label: {
                    HStack(spacing: 14) {
            ...
            Text("Purple: completed · Orange: skipped · Gray: pending. Tap a day to open its journal.")
        }
    }
            ...
            if items.isEmpty {
                ...
                HStack(spacing: 0) {
                    Rectangle()
                        ...
                        .frame(
                            width: geometry.size.width
                                * CGFloat(done) / CGFloat(items.count)
                        )
                    ...
                    Rectangle()
                        ...
                        .frame(
                            width: geometry.size.width
                                * CGFloat(skippedCount) / CGFloat(items.count)
                        )
                    ...
                    Rectangle()
                        ...
                        .frame(
                            width: geometry.size.width
                                * CGFloat(pendingCount) / CGFloat(items.count)
                        )
                }
                .clipShape(Capsule())
            }
    ...
    private func entry(for day: Date) -> JournalEntry? {
        entries.first {
            $0.dayKey == JournalEntry.key(for: day)
        }
    }
